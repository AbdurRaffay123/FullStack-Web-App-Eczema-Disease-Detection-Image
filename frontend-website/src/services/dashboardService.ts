...
class DashboardService {
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    try {
      ...
      const eczemaDetectedCount = images.filter(img => img.analysisResult?.eczema_detected).length;
      ...
      images.slice(0, 3).forEach(image => {
        ...
        activities.push({
          ...
          description: hasEczema 
            ? `${severity || 'Eczema'} detected - ${(image.analysisResult?.confidence * 100 || 0).toFixed(0)}% confidence`
            : 'No eczema detected',
